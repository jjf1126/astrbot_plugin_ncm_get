...
@register("astrbot_plugin_ncm_get", "AstrBot Developer", "解析网易云/QQ音乐链接，支持短链接深度解析与自主工具。", "2.3.1", "https://github.com/yourusername/astrbot_plugin_ncm_get")
class MusicGetPlugin(Star):
    ...
    async def _fetch_qq_detail(self, songmid: str):
        url = f"https://c.y.qq.com/v8/fcg-bin/fcg_play_single_song.fcg?songmid={songmid}&tmpl=v2.0&format=json"
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, headers=headers, allow_redirects=True, timeout=5) as resp:
                    ...
                    match_fallback = re.search(r'(00[a-zA-Z0-9]{12})', html_text)
                    if match_fallback: return match_fallback.group(1)
        except Exception as e:
            logger.error(f"解析 QQ 短链接深度穿透失败: {e}")
        return ""
    ...
    async def set_ncm_cookie(self, event: AstrMessageEvent, cookie: str = ""):
        '''设置网易云音乐 Cookie'''
        if not cookie:
            yield event.plain_result("请提供有效的网易云 Cookie 字符串。例如: /ncm_cookie MUSIC_U=xxxx;")
            return
        self.ncm_cookie = cookie
        yield event.plain_result(f"网易云 Cookie 已更新成功！当前长度为：{len(cookie)}")

    @filter.command("qq_cookie")
    async def set_qq_cookie(self, event: AstrMessageEvent, cookie: str = ""):
        '''设置QQ音乐 Cookie'''
        if not cookie:
            yield event.plain_result("请提供有效的 QQ 音乐 Cookie 字符串。例如: /qq_cookie qm_keyst=xxxx; uin=yyyy;")
            return
        self.qq_cookie = cookie
        yield event.plain_result(f"QQ 音乐 Cookie 已更新成功！当前长度为：{len(cookie)}")
    ...
    async def ncm_get(self, event: AstrMessageEvent, url: str):
        ...
        if not song_id:
            yield event.plain_result("未识别到有效的网易云音乐链接。")
            return
    ...
    async def qq_get(self, event: AstrMessageEvent, url: str):
        '''解析QQ音乐链接'''
        songmid = await self._resolve_qq_url(url)
        if not songmid:
            yield event.plain_result("未识别到有效的QQ音乐链接。")
            return
        ...
        try:
            ...
            if qq_urls:
                ...
                if smid:
                    ...
                    req.system_prompt += f"\n\n{self.inject_format.format(title=t, artist=a, lyrics=l, song_id=smid, iframe='')}"
                    logger.info(f"成功注入QQ音乐: 《{t}》")
            ...
            if ncm_urls:
                ...
                if sid:
                    ...
                    req.system_prompt += f"\n\n{self.inject_format.format(title=t, artist=a, lyrics=l, song_id=sid, iframe='')}"
                    logger.info(f"成功注入网易云: 《{t}》")
        except Exception as e:
    ...
    async def get_qq_song_info(self, event: AstrMessageEvent, identifier: str):
        """
        获取QQ音乐歌曲的详细信息和歌词。当用户发出QQ音乐链接或询问QQ音乐歌曲时调用。
        
        Args:
            identifier (string): 完整的QQ音乐URL链接，或者是歌曲的songmid（如 0039MnYb0qxYhV）。
        """
        try:
            ...
            if not clean_id: return {"error": "无效的QQ音乐链接或ID"}
    ...
    async def get_ncm_song_info(self, event: AstrMessageEvent, identifier: str):
        """
        获取网易云音乐歌曲的详细信息和歌词。当用户发出网易云链接或询问网易云歌曲时调用。
        
        Args:
            identifier (string): 完整的网易云URL链接，或者是歌曲的纯数字ID。
        """
        try:
            ...
            if not clean_id: return {"error": "无效的网易云链接或ID"}
